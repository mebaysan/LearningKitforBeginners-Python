...
kelime = "Python ile regex kullanımı nasıl olmalıdır sizce ?"

result = re.findall("regex",kelime) # aradığımız kelime veya pattern'i 2. parametre içerisinde arar ve bir liste olarak döner

result = re.split(" ",kelime) # ilk  parametrede verdiğimiz kelime || harf || işaret gibi nesnelerle 2. parametreyi böler

result = re.sub(" ","-",kelime) # 1. parametrede bulduğun her şeyi 2. parametredeki değer ile değiştir ve bunu 3. parametre içerisinde yap

result = re.search("Python",kelime) # ilk parametreyi ikinci içerisidne arar. span kısmında kaçıncı index ile kaçıncı index arasında bulduğunu söyler
#result = result.span() # index numaralarını bu şekilde alabiliriz
#result = result.start() # başlangıç numarasını verir
#result = result.end() # bitiş numarasını verir
#result = result.group() # bulduğu ifadeyi bize verir
#result = result.string # hangi string içerisinde aradğını verir
