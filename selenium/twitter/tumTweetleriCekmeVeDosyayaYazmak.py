...
tweets = list() # tweetleri tutacağımız bir liste oluşturduk

for element in elements: # elements listemizde geziniyoruz
    tweets.append(element.text) # tweets listemize elementlerin text değerlerini ekliyoruz

tweetCount = 1 # güzel görünüm için oluşturduğumuz değişken
with open("tweetler.txt","w",encoding="UTF-8") as file: # bir dosya açıyoruz yazma kipiyle
    for tweet in tweets: # tweets listemizdeki her eleman için
        file.write(str(tweetCount)+". Tweet\n"+tweet+"\n****************") # dosyaya yazıyoruz
        tweetCount+=1 # her seferinde tweetCount u bir artırıyoruz
